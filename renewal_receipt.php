...
if (isset($_REQUEST['add'])){
	...
	if($InvoiceHeaderID==0){
		...
		$sql="select il.ServiceHeaderID,c.CustomerName,il.Amount,isnull(sum(rl.Amount),0) Paid,il.Amount-isnull(rl.Amount,0) Balance
		from (select InvoiceHeaderID,ServiceHeaderID,sum(Amount) Amount from  LicenceRenewalnvoiceLines group  by InvoiceHeaderID,ServiceHeaderID) il
		join ServiceHeader sh on il.ServiceHeaderID=sh.ServiceHeaderID
		join Customer c on sh.CustomerID=c.CustomerID
		left join (SELECT InvoiceHeaderID,sum(Amount)Amount FROM LicenceRenewaReceiptLines group by InvoiceHeaderID) rl on rl.InvoiceHeaderID=il.InvoiceHeaderID
		where il.InvoiceHeaderID=$InvoiceHeaderID
		group by il.InvoiceHeaderID,il.ServiceHeaderID,c.CustomerName,il.Amount,rl.Amount";
		$Details=sqlsrv_query($db,$sql);
		while($row=sqlsrv_fetch_array($Details,SQLSRV_FETCH_ASSOC)){
			$Customer=$row['CustomerName'];
			$InvoiceAmount=$row['Amount'];
...
if ($_REQUEST['Receipt']==1)
{
	...
	}else
	{
		if($InvoiceHeaderID!==0 and $ReceiptAmount!==0){
			...
			{	
				$result=ReceiptLicenceRenewalMoney($db,$DateReceived,$BankID,$RefNumber,$PaymentMethod,$InvoiceHeaderID,$SlipAmount,$ReceiptAmount,$CreatedUserID);
			}
		}
		if($InvoiceHeaderID1!==0 and $ReceiptAmount1!==0){
			if((double)$ReceiptAmount1>0)
			{	
				$result=ReceiptLicenceRenewalMoney($db,$DateReceived,$BankID,$RefNumber,$PaymentMethod,$InvoiceHeaderID1,$SlipAmount,$ReceiptAmount1,$CreatedUserID);
			}
		}
		if($InvoiceHeaderID2!==0 and $ReceiptAmount2!==0){
			if((double)$ReceiptAmount2>0)
			{	
				$result=ReceiptLicenceRenewalMoney($db,$DateReceived,$BankID,$RefNumber,$PaymentMethod,$InvoiceHeaderID2,$SlipAmount,$ReceiptAmount2,$CreatedUserID);
			}
		}
		if($InvoiceHeaderID3!==0 and $ReceiptAmount3!==0){	
			if((double)$ReceiptAmount3>0)
			{
				$result=ReceiptLicenceRenewalMoney($db,$DateReceived,$BankID,$RefNumber,$PaymentMethod,$InvoiceHeaderID3,$SlipAmount,$ReceiptAmount3,$CreatedUserID);
			}
		}
		if($InvoiceHeaderID4!==0 and $ReceiptAmount4!==0){	
			if((double)$ReceiptAmount4>0)
			{
				$result=ReceiptLicenceRenewalMoney($db,$DateReceived,$BankID,$RefNumber,$PaymentMethod,$InvoiceHeaderID4,$SlipAmount,$ReceiptAmount4,$CreatedUserID);
			}
		}
...
<div class="example">
      ...
      <table class="ui selectable attached basic table">
        ...
        <tbody id="tbody"> 
        	...
        	<tr>
              ...
              <td>
              <input name="Button" type="button" onclick="loadmypage('renewal_receipt.php?'+
              										'&remove=1'+
	...
	<div class="field">
		<input name="btnReceipt" type="button" Value="Save" class="ui button" onclick="loadmypage('renewal_receipt.php?'+							
