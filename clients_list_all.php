...
if (isset($_REQUEST['save']) && $_REQUEST['NextStatus']!='')
{
	...
	{	
			...
			else
			{
				...
				}else
				{
					...
					{	
						...
						// $feedBack=createInvoice($db,$ApplicationID,$cosmasRow,$Remark,$CustomerName,$InvoiceHeader);
		...
		else if ($NextStatusID==7)
		{
			...
			$sql="Select InvoiceHeaderID,Balance From vwPayments where ServiceHeaderID=$ApplicationID";
			$s_result = sqlsrv_query($db, $sql);
			if ($s_result)
			{	
				while ($row = sqlsrv_fetch_array( $s_result, SQLSRV_FETCH_ASSOC))
			...
			}else
			{
				...
				{ */	
					...
					if ($s_result1)
					{
						...
						// $feedBack=createPermit($db,$ApplicationID,$cosmasRow);
						// $msg=$feedBack[1];
						// $mail=true;
						/* if($feedBack[0]=="true")
						{
					...
					<td colspan="6">
						<table width="100%">
							<tr>
								...
								<td><label>&nbsp;</label>
								<input name="btnSearch" type="button" onclick="loadmypage('clients_list_all.php?'+
											'&fromDate='+this.form.fromDate.value+								
											'&toDate='+this.form.toDate.value+
											'&search=1','content','loader','listpages','','applications_all','rolecenter=<?php echo $_SESSION['RoleCenter']; ?>:fromDate='+this.form.fromDate.value+':toDate='+this.form.toDate.value+':ServiceHeaderID='+this.form.ServiceHeaderID.value+':CustomerName='+this.form.CustomerName.value+'')" value="Search">
								</td>
							</tr>
						</table>
					</td>							  
