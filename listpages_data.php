...
if (isset($_REQUEST['exParam'])) 
{ 
	$exParam =$_REQUEST['exParam']; 
}else{
	$exParam='None';
}
...
else if ($OptionValue == 'ServiceGroups')
{
	...
	while($myrow = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC))
	{
		...
		$ChecklistBtn = '<a href="#" onClick="loadmypage(\'servicechecklist_list.php?ServiceGroupID='.$ServiceGroupID.'\',\'content\',\'loader\',\'listpages\',\'\',\'ServiceChecklist\',\''.$ServiceGroupID.'\')">Checklist</a>';
		...
		$Actions='['.$EditBtn.'|'.$ChecklistBtn.'|'.$DeleteBtn.']';
...
else if($OptionValue=='licence-application-invoices-a')
{
	// echo '<pre>';
	// print_r($exParam);
	// exit;
...
else if($OptionValue=='classificationapplications')
{
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	...
	set dateformat dmy SELECT top 100 sh.ServiceHeaderID 
				  ...
				  inner join ServiceGroup sg on sc.ServiceGroupID = sg.ServiceGroupID 
				  INNER JOIN dbo.Forms f on sh.FormID=f.FormID 
				  where sh.ServiceStatusID =1 and sg.ServiceGroupID = 11 or (sh.ServiceStatusID = 5) and (sc.InvoiceStage<>sc.LastStage or sh.ServiceStatusID<>sc.LastStage) 
				  and sh.ServiceID not in (select ServiceID from ServiceTrees) order by sh.SubmissionDate desc
...
else if($OptionValue=='LicenceApplicationApprovedByOfficer')
{
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	...
	set dateformat dmy SELECT top 100 sh.ServiceHeaderID 
				  ...
				  inner join ServiceGroup sg on sc.ServiceGroupID = sg.ServiceGroupID 
				  INNER JOIN dbo.Forms f on sh.FormID=f.FormID 
				  where sh.ServiceStatusID =12 and c.BusinessZone= $RegionID or (sh.ServiceStatusID = 5 and sh.ServiceCategoryID = 2033) and (sc.InvoiceStage<>sc.LastStage or sh.ServiceStatusID<>sc.LastStage) 
				  and sh.ServiceID not in (select ServiceID from ServiceTrees) order by sh.SubmissionDate desc
...
else if($OptionValue=='classificationapproval')
{
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	...
	set dateformat dmy SELECT top 100 sh.ServiceHeaderID 
				  ...
				  inner join ServiceGroup sg on sc.ServiceGroupID = sg.ServiceGroupID 
				  INNER JOIN dbo.Forms f on sh.FormID=f.FormID 
				  where sh.ServiceStatusID =12 and sc.ServiceGroupID = 11 or (sh.ServiceStatusID = 5 and sh.ServiceCategoryID = 2033) and (sc.InvoiceStage<>sc.LastStage or sh.ServiceStatusID<>sc.LastStage) 
				  and sh.ServiceID not in (select ServiceID from ServiceTrees) order by sh.SubmissionDate desc
...
else if($OptionValue=='renewalapplications')
{
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	...
	$msql = "set dateformat dmy SELECT top 100 sh.LicenceId AS ApplicationID,sh.LicenceRenewalStatusId as ServiceStatusID,ss.ServiceStatusName, 
	s.ServiceName ,c.CustomerID, c.CustomerName, sh.SubmissionDate,s.ServiceID,s.ServiceCategoryID,s.ServiceCategoryID 

	FROM dbo.LicenceRenewals AS sh INNER JOIN 
...
else if($OptionValue=='approved_pending_approval')
{
	// exit('hap');
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
...
else if($OptionValue=='approved_classification_approval')
{
	// exit('hap');
	$params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
...
else if($OptionValue=='Inspections')
{
	$fromDate;
	$toDate;
...
=======
			// echo $sql1;exit;
>>>>>>> 4d89b0ea5569eafe483387123d6ececdd45592b5
		// "set dateformat dmy 
...
else if($OptionValue=='InspectionResults')
{
	$psql = "select cat.ParameterCategoryID, cat.ParameterCategoryName,cat.ParameterCategoryDescription
			from ChecklistParameterCategories cat 
			where ParameterCategoryID in (select distinct cp.ParameterCategoryID
			from ChecklistResults cr
			join ChecklistParameters cp on cr.ParameterID=cp.ParameterID
			where cr.InspectionID=$exParam)";
// echo $psql;exit;
	$result = sqlsrv_query($db, $psql);	
	$i=1;
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		extract($row);	

		$sql="select  cp.ParameterName ComplianceItem,cr.ParameterID,ro.ResultValue,cr.ResultComment,cr.ParameterScore
				from ChecklistResults cr
				join ChecklistParameters cp on cr.ParameterID=cp.ParameterID
				join ResultOptions ro on cr.ResultOptionID=ro.ResultOptionID
				where cr.InspectionID='$exParam' and cp.ParameterCategoryID=$ParameterCategoryID";
...
// echo $sql;
		...
		while ($cp = sqlsrv_fetch_array( $result2, SQLSRV_FETCH_ASSOC)) 
		{
			extract($cp);
			$tr.='<tr>';
			$td='<td width="30%">'.$ComplianceItem.'</td>';
			$td.='<td width="10%">
		...
		(
			...
			'<div>'.
				...
				'<div><strong>Parameter Question:</strong> '.$ParameterCategoryDescription.'</div>'.						
			'</div>',
			'<div>'.$table.'</div>'
		);
		$i+=1;	
...
else if ($OptionValue=='LicenceRenewalForm')
{
	...
	{
		extract($row);
		$ColumnBtn ='<a href="#" onClick="loadmypage(\'licence_renewal_form_columns_list.php?FormID='.$FormID.'&FormName='.$FormName.'\',\'content\',\'loader\',\'listpages\',\'\',\'LicenceRenewalFormColumns\','.$FormID.')">Columns</a>';
		$SectionBtn ='<a href="#" onClick="loadmypage(\'licence_renewal_form_sections_list.php?FormID='.$FormID.'&FormName='.$FormName.'\',\'content\',\'loader\',\'listpages\',\'\',\'LicenceRenewalFormSections\','.$FormID.')">Sections</a>';
		$EditBtn = '<a href="#" onClick="loadpage(\'form.php?edit=1&FormID='.$FormID.'\',\'content\')">Edit</a>';
		// $DeleteBtn  = '<a href="#" onClick="deleteConfirm2(\'Are you sure you want to Delete?\',\'forms_list.php?delete=1&FormID='.$FormID.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValue.'\')">Delete</a>';
		$actions='['.$ColumnBtn.'|'.$SectionBtn.'|'.$EditBtn.'|'.$DeleteBtn.']';
...
else if ($OptionValue=='LicenceRenewalFormSections')
{
	$sql = "select LicenceRenewalFormSection.*,LicenenceRenewalForm.FormName from LicenceRenewalFormSection  inner join LicenenceRenewalForm  on LicenceRenewalFormSection.LicenceRenewalFormID=LicenenceRenewalForm.FormId where LicenenceRenewalForm.FormId=$exParam";
	//echo $sql;		
	$result = sqlsrv_query($db, $sql);	
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		...
		$EditBtn = '<a href="#" onClick="loadpage(\'licence_renewal_form_sections_card.php?edit=1&FormSectionID='.$FormSectionID.'&FormName='.$FormName.'\',\'content\')">Edit</a>';
		$DeleteBtn  = '<a href="#" onClick="deleteConfirm2(\'Are you sure you want to Delete?\',\'form_sections_list.php?delete=1&FormSectionID='.$FormSectionID.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValue.'\')">Delete</a>';
		$actions='['.$EditBtn.'|'.$DeleteBtn.']';
...
else if ($OptionValue=='LicenceRenewalFormColumns')
{
	$sql = "select fc.*,f.FormName,fs.FormSectionName,fc.ColumnSize,cdt.ColumnDataTypeName from LicenceRenewalColumns fc 
		inner join LicenenceRenewalForm f on fc.LicenceRenewalFormID=f.FormID
		left join ColumnDataType cdt on fc.ColumnDataTypeID=cdt.ColumnDataTypeID
		left join LicenceRenewalFormSection fs on fc.FormSectionID=fs.FormSectionID
		where f.FormID=$exParam";
		// exit($sql);
...
else if($OptionValue=='Counties')
{
	...
	$result = sqlsrv_query($db, $sql);
	$OptionValueAgain = 'RequestTypes';	
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		extract($row);
		$EditBtn = '<a href="#" onClick="loadpage(\'county.php?edit=1&CountyID='.$CountyId.'\',\'content\')">Edit</a>';			
		// $RequestTypes = '<a href="#" onClick="loadpage(\'Request_Type.php?edit=1&CountyID='.$CountyId.'\',\'content\')">Request Types</a>';			
		$RequestTypes = '<a href="#" onClick="LoadAnotherTable(\'Are you sure you want to Delete?\',\'RequestType_list.php?delete=1&CountyID='.$CountyId.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValueAgain.'\')">Request Types</a>';

		$actions='['.$EditBtn.'||'.$RequestTypes.']';
		$StartDate=date("d/m/Y",strtotime($StartDate));
		$EndDate=date("d/m/Y",strtotime($EndDate));
		$channel[] = array(			
					$CountyName,
...
else if($OptionValue=='RequestTypes')
{
	$sql = "select *  from RequestTypes as RTY 
	join SubSystems as SubSys on SubSys.SubSystemID = RTY.RegionId 
	WHERE RTY.RegionId = 1"; //$CountyId
	$result = sqlsrv_query($db, $sql);
	$OptionValueAgain='RequestTypeApprovers';
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		extract($row);
		$EditBtn = '<a href="#" onClick="loadpage(\'county.php?edit=1&RequestTypeID='.$RequestTypeID.'\',\'content\')">Edit</a>';			
		// $RequestTypes = '<a href="#" onClick="loadpage(\'Request_Type.php?edit=1&CountyID='.$CountyId.'\',\'content\')">Request Types</a>';			
		$RequestApprovers = '<a href="#" onClick="LoadAnotherTable(\'Are you sure you want to Delete?\',\'RequestApprovers.php?delete=1&CountyID='.$CountyId.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValueAgain.'\')">Request Approvers</a>';
...
else if($OptionValue=='RequestTypeApprovers')
{
	...
	WHERE RTY.RequestTypeID = 14"; //$CountyId
	// exit($sql);
	$result = sqlsrv_query($db, $sql);
	$OptionValueAgain=='RequestTypeApprovers';
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		extract($row);
		$EditBtn = '<a href="#" onClick="loadpage(\'ApproverSetUpCard.php?edit=1&ApproverId='.$ApproverID.'\',\'content\')">Edit</a>';			
		// $RequestTypes = '<a href="#" onClick="loadpage(\'Request_Type.php?edit=1&CountyID='.$CountyId.'\',\'content\')">Request Types</a>';			
		$DeleteApprover = '<a href="#" onClick="deleteConfirm2(\'Are you sure you want to Remove the Approver?\',\'RequestApprovers.php?delete=1&ID='.$ID.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValueAgain.'\')">Remove Approver</a>';

		$actions='['.$EditBtn.'||'.$DeleteApprover.']';
...
else if($OptionValue=='ChecklistTypes')
{
	...
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		...
		$EditBtn = '<a href="#" onClick="loadpage(\'checklisttype.php?edit=1&ChecklistTypeID='.$ChecklistTypeID.'\',\'content\')">Edit</a>';
		$DeleteBtn = '<a href="#" onClick="deleteConfirm2(\'Are you sure you want to Delete?\',\'checklisttype_list.php?delete=1&ChecklistTypeID='.$ChecklistTypeID.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValue.'\','.$ChecklistTypeID.')">Delete</a>';
...
{
	$sql = "select cp.ParameterID,ParameterName,ParameterCategoryName,ParameterScore,ct.ChecklistTypeName 
			from ChecklistParameters cp
			join ChecklistParameterCategories cpc on cp.ParameterCategoryID=cpc.ParameterCategoryID
			join ChecklistTypes ct on cp.ChecklistTypeID=ct.ChecklistTypeID";
...
else if($OptionValue=='ServiceChecklist')
{
	$sql = "select ac.ServiceChecklistID, cp.ParameterName,cpc.ParameterCategoryName,cpc.ParameterCategoryDescription,s.ServiceGroupName
			from ServiceChecklists ac
			join ChecklistParameters cp on ac.ParameterID=cp.ParameterID
			join ChecklistParameterCategories cpc on cp.ParameterCategoryID=cpc.ParameterCategoryID
			join ServiceGroup s on ac.ServiceGroupID=s.ServiceGroupID
			where ac.ServiceGroupID=$exParam 
			order by s.ServiceGroupName";
	...
	while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)) 
	{
		...
		$EditBtn ='';// '<a href="#" onClick="loadpage(\'service_charges.php?edit=1&ServiceID='.$exParam.'&SubSystemID='.$SubSystemId.'&FinancialYearID='.$FinancialYearId.'&ServiceAmount='.$Amount.'\',\'content\')">Edit</a>';
		$DeleteBtn   = '<a href="#" onClick="deleteConfirm2(\'Are you sure you want to Delete?\',\'servicechecklist_list.php?delete=1&ServiceChecklistID='.$ServiceChecklistID.'\',\'content\',\'loader\',\'listpages\',\'\',\''.$OptionValue.'\',\''.$exParam.'\')">Delete</a>';
