...
if (isset($_REQUEST['save']))
{
	...
	$CustomerName=$_REQUEST['ApplicantName'];
	$LocalAuthorityID=$_REQUEST['LocalAuthorityID'];
	$MobileNo=$_REQUEST['ApplicantMobile'];
	$Address=$_REQUEST['ApplicantAddress'];
	...
	$sql="select l.upn, sh.CustomerID,la.LRN lrn,la.PlotNo plotno 
	from land l 
		...
		if ($result3)
		{
			...
			if($result)
			{
				while($rw=sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC))
				{
					...
					$sql="Insert into Miscellaneous (ServiceHeaderID,CustomerName,IDNo,MobileNo,Address,Description,Amount,CreatedBy)
					Values('$ServiceHeaderID','$CustomerName','$ApplicantIDNo','$ApplicantMobile','$applicantaddress','$Description','$Total','$CreatedBy')";
					//echo $sql;
...
<form>
	<fieldset>
	  <legend>Building Plan Invoicing</legend>
          				...
          				<table width="100%">
          					<tr>
          						<td>
          							<label>Name</label>
				                    <div class="input-control text" data-role="input-control">
				                        <input type="text" id="ApplicantName" name="ApplicantName" value="<?php echo $ApplicantName; ?>"></input>
				                        <button class="btn-clear" tabindex="-1"></button>
				                    </div>
          						</td>
          					</tr>
          					<tr>
          						<td>
          							<label>ID NO</label>
				                    <div class="input-control text" data-role="input-control">
				                        <input type="text" id="ApplicantIDNo" name="ApplicantIDNo" value="<?php echo $ApplicantIDNo; ?>"></input>
				                        <button class="btn-clear" tabindex="-1"></button>
				                    </div>
          						</td>
          					</tr>
          					<tr>
          						<td>
          							<label>Address</label>
				                    <div class="input-control text" data-role="input-control">
				                        <input type="text" id="ApplicantAddress" name="ApplicantAddress" value="<?php echo $ApplicantAddress; ?>"></input>
				                        <button class="btn-clear" tabindex="-1"></button>
				                    </div>
          						</td>
          					</tr>
          					<tr>
          						<td>
				                    ...
				                    <div class="input-control text" data-role="input-control">
				                        <input type="text" id="ApplicantMobile" name="ApplicantMobile" value="<?php echo $ApplicantMobile; ?>"></input>
				                        <button class="btn-clear" tabindex="-1"></button>
				                    </div>
          						</td>
          					</tr>
		...
		FencingTypeID=this.form.FencingTypeID.selected;
		CustName=this.form.CustomerName.value;
		RatesBalance=this.form.balance.value;
		Registered=this.form.registered.value;
		applicantname=this.form.ApplicantName.value;
		applicantmobile=this.form.ApplicantMobile.value;
		applicantidno=this.form.ApplicantIDNo.value;
		applicantaddress=this.form.ApplicantAddress.value;
		...
		if(Registered=='0'){
			...
			exit;
		}else if(FencingTypeID==''){
			alert('The Fencing type must be stated');
			exit;
		}else if(applicantname==''){
			alert('The Applicant Name must be stated');
			exit;
		}else if(applicantmobile==''){
			alert('The Applicant Mobile No. must be stated');
			exit;
		}else if(applicantaddress==''){
			alert('The Applicant Address must be stated');
			exit;
		}else if(applicantidno==''){
			alert('The Applicant ID No. must be stated');
			exit;
		}

		loadmypage('fencing_application_b.php?'+
